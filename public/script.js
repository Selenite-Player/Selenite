...
function play() {
  ...
  setTimeout(
    () =>
      helper.toggleClass(document.getElementById("play"), [
        "fa fa-play",
        "fa fa-pause",
      ]),
    200
...
function shuffle() {
  ...
  helper.toggleClass(document.getElementById("shuffle"), [
    "fa fa-random active",
    "fa fa-random",
  ]);
}
...
function repeat() {
  ...
  setTimeout(
    () =>
      (document.getElementById("repeat").className = helper.getRepeatClassName(
        state.repeat_options[index]
      )),
    200
  );
}
...
function saveSong() {
  if (state.song_id != null) {
    state.saved
      ? ipcRenderer.send("delete-song", state.song_id)
      : ipcRenderer.send("save-song", state.song_id);
