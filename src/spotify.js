...
let DEVICE_ID = settings.getSync("device_id");
...
function getDeviceId() {
  return _fetchResult("https://api.spotify.com/v1/me/player/devices", "GET")
    .then((data) => {
      let id = data.devices.filter((device) => device.type === "Computer")[0]
        .id;
      return id;
    })
...
function getCurrentlyPlaying() {
  return _fetchResult(
    `https://api.spotify.com/v1/me/player?additional_types=episode&device_id=${DEVICE_ID}`,
    "GET"
  );
}
...
function resume() {
  return _fetch(
    `https://api.spotify.com/v1/me/player/play?device_id=${settings.getSync(
      "device_id"
    )}`,
    "PUT"
  );
...
function pause() {
  return _fetch(
    `https://api.spotify.com/v1/me/player/pause?device_id=${settings.getSync(
      "device_id"
    )}`,
    "PUT"
  );
